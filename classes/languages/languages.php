...
class WcvaLanguage 
{
    ...
    const WIDGET_MESSAGES = 'wcvaWidgetMessages';
    const PLACEHOLDER = 'placeholder';

    const ERRORS = 'wcvaErrorLibrary';
     ...
     * 
     * @return Array $wcva_all_lang_set  Array of language arrays
     */
    public static function get_all_languages() {
        $wcva_all_lang_set = array();
        
        try {
    
            if (!empty(self::$gcp_language_set)) {
                $wcva_all_lang_set = self::$gcp_language_set;
            } else {
                $wcva_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
            }
    
            uksort($wcva_all_lang_set, 'strcasecmp');
        } catch(\Exception $err) {
            $wcva_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
        }

        return $wcva_all_lang_set;
    }
    ...
    public static function get_short_phrase_lang_code($lang_code) {
        ...
        $wcva_valid_lang_code = false;

        if (self::gcp_supported($lang_code)) {
            $wcva_lang = self::$gcp_language_set[$lang_code];
            $wcva_valid_lang_code = true;
        }

        if ($wcva_valid_lang_code === true && array_key_exists(self::SHORT_PHRASE_LANG, $wcva_lang)) {
            $short_phrase_lang = $wcva_lang[self::SHORT_PHRASE_LANG];
        }
